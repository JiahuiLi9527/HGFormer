...
class RandomVerticalFlip(object):
    ...
    def __call__(self, img, mask, high_image):
        if random.random() < self.p:
            img = F.vflip(img)
            mask = F.vflip(mask)
            high_image = F.vflip(high_image)
        return img, mask, high_image
...
class RandomRotate(object):
    ...
    def __call__(self, img, mask, high_image):
        if random.random() < self.p:
            ...
            return F.rotate(img, angle, self.resample, self.expand, self.center), F.rotate(mask, angle, self.resample, self.expand, self.center), F.rotate(high_image, angle, self.resample, self.expand, self.center)
        return img, mask, high_image
...
class PolypDataset(Dataset):
    ...
    def resize(self, img, gt, high_image):
        assert img.size == gt.size == high_image.size
        w, h = img.size
        if h < self.trainsize or w < self.trainsize:
            h = max(h, self.trainsize)
            w = max(w, self.trainsize)
            return img.resize((w, h), Image.BILINEAR), gt.resize((w, h), Image.NEAREST), high_image.resize((w, h), Image.BILINEAR)
        else:
            return img, gt, high_image
